function descargarPDF70() {
    const clvResult = document.getElementById('result-markov').innerText;
    ...
    doc.setFont('Helvetica', 'bold');
    const additionalText = "RESULTADOS DE TU CALCULO CLTV | MARKOV";
    doc.setFontSize(12);
    ...
    doc.setFont('Helvetica', 'normal');
    doc.setFontSize(12);
    doc.text(`Probabilidad de estar en el Estado 1 (en %): ${document.getElementById('state1-markov').value}`, 20, y);
    y += lineHeight;
    doc.text(`Valor esperado en Estado 1: ${document.getElementById('value1-markov').value}`, 20, y);
    y += lineHeight;
    doc.text(`Probabilidad de estar en el Estado 2 (en %): ${document.getElementById('state2-markov').value}`, 20, y);
    y += lineHeight;
    doc.text(`Valor esperado en Estado 2: ${document.getElementById('value2-markov').value}`, 20, y);
    y += lineHeight;
    doc.text(`Probabilidad de estar en el Estado 3 (en %): ${document.getElementById('state3-markov').value}`, 20, y);
    y += lineHeight;
    doc.text(`Valor esperado en Estado 3: ${document.getElementById('value3-markov').value}`, 20, y);
    y += lineHeight;
    ...
    doc.setLineWidth(0.5);
    doc.rect(15, 55, 180, y - 60); // Cambia el tamaño del rectángulo según sea necesario
    ...
    doc.text(contactText, contacttextX, 135);



    // Guardar el PDF
    doc.save('Resultado_CLTV_Markov.pdf');
}
